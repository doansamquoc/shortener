...
public class AuthController {
	...
	private ResponseEntity<ApiResponse<?>> forgotPassword(
		@Valid @RequestBody ForgotPasswordRequest request
	) {
		service.forgotPassword(request.email());
		return ResponseEntity.ok(ApiResponse.of(
			"If the email valid, we'll send a verification email!"));
	}
	...
	private ResponseEntity<ApiResponse<VerifyResetCodeResponse>> verifyResetCode(
		...
		VerifyResetCodeResponse response = service.verifyResetPasswordCode(
			request.email(),
			request.code()
		);
		return ResponseEntity.ok(ApiResponse.of(response));
