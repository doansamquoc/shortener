...
public class AuthServiceImpl implements AuthService {
    ...
    public TokenDto register(UserRegistrationRequest request) {
        String hashedPassword = passwordEncoder.encode(request.password()); User user =
            userService.create(new UserRegistrationRequest(request.email(), request.username(), hashedPassword));
        
        RefreshToken refreshToken = refreshTokenService.create(user);
        String accessToken = generateToken(fromUser(user));
        
        publisher.publishEvent(new UserRegisteredEvent(user)); return new TokenDto(accessToken, refreshToken);
    }
    ...
    public TokenDto exchangeToken(ExchangeTokenRequest request) {
        ...
        CustomOAuth2User oauth2User = (CustomOAuth2User) userObj; authTemporaryCode.remove(request.code());
        
        User user = oauth2User.createUser(); RefreshToken refreshToken = refreshTokenService.create(user);
    ...
    public TokenDto refreshToken(String token) {
        ...
        refreshTokenService.revoke(refreshToken); User user = refreshToken.getUser();
    ...
    public void forgotPassword(String email) {
        userService.findByEmail(email).ifPresent(u -> {
            ...
            forgotPasswordCache.add(email, code, duration); publisher.publishEvent(new ForgotPasswordEvent(u, code));
        });
    }
    ...
    public VerifyResetCodeResponse verifyResetPasswordCode(String email, String code) {
        ...
        forgotPasswordCache.remove(email); String resetToken = UUID.randomUUID().toString();
        
        long duration = System.currentTimeMillis() + props.getResetPasswordTokenExpiration();
        resetTokenCache.add(resetToken, email, duration); return new VerifyResetCodeResponse(resetToken);
    }
    ...
    public void resetPassword(String token, ResetPasswordRequest request) {
        ...
        userService.findByEmail(email).ifPresentOrElse(
            user -> {
                String hashedPassword = passwordEncoder.encode(request.password()); user.setPassword(hashedPassword);
                userService.save(user);
                
                resetTokenCache.remove(email); refreshTokenService.revokeAll(user);
            }, () -> {
